...
import io.camunda.kiwi.reviewer.review.RuleEvaluationResult.Details.ExpectedValue;
import io.camunda.kiwi.reviewer.review.RuleEvaluationResult.Details.UnexpectedValue;
...
public class ReviewExecution {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ReviewExecution.class);
    ...
    public final ReviewReponse reviewValues(String helmVersion, Map<String, Object>... valuesUnmerged) {
        ...
        return reviewMergedValues(helmVersion, values);
    }


    /**
     * Run the execution.
     * If any error is detected during the execution, it create a new RuleEvaluationResultGroup at the end with all information
     *
     * @param helmVersion
     * @param values
     * @return
     */
    public final ReviewReponse reviewMergedValues(String helmVersion, Map<String, Object> values) {
        Version version = versionRangeParser.parse(helmVersion).as(Equals.class).version();
        ...
        nonExistentProperties(values, version, results, failures, collectorInformation);

        onlyNonDefaults(values, version, results, failures, collectorInformation);
        if (!failures.isEmpty()) {
            logger.warn(
                    "There are failures: \n{}",
        ...
        ReviewReponse reviewResult = new ReviewReponse();
        reviewResult.rulesEvaluation = results;
        reviewResult.executionInformations = collectorInformation.getInformations();
     ...
     * small
     */
    private void onlyNonDefaults(Map<String, Object> values,
                                 Version version,
        ...
        Map<String, Object> defaultValues = buildDefaultValues(values,
                "camunda",
                "camunda-platform",
                version.toString(),
                null,
    ...
    /**
     * Run UpgraderRules specified in RulesInitializer
     */
                ...
                .forEach(
                        (groupId, group) -> {
                            ...
                            if (!resultsForGroup.isEmpty()) {
                                results.add(
                                        new RuleEvaluationResultGroup(groupId, group.name(), group.description(), resultsForGroup, null));
                            }
                        });
    }
    ...
    void nonExistentProperties(
        ...
        Map<String, Object> defaultValuesAsMap = buildDefaultValues(
                values,
                "camunda",
                "camunda-platform",
                version.toString(),
     ...
     * @return
     */
    private Map<String, Object> buildDefaultValues(Map<String, Object> values,
                                                   String repoName,
                                                   String chartName,
                                                   String chartVersion,
                                                   String path,
                                                   CollectorInformation collectorInformation) {
        if (path == null) {
            logger.debug("Building default values for {}/{}:{}", repoName, chartName, chartVersion);
        }

        logger.info("BuildDefaultValues repo[{}] chartName[{}] version[{}] path[{}]", repoName, chartName, chartVersion, path);
        collectorInformation.reportUniqKey(chartName + "#" + chartVersion, CollectorInformation.LEVEL.INFO, "Load ChartName[" + chartName + "] version[" + chartVersion + "] repo[" + repoName + "]");

        Map<String, Object> defaultValuesAsMap = helmService.showValues(repoName, chartName, chartVersion);
        Map<String, Object> chart = helmService.showChart(repoName, chartName, chartVersion);
        ...
        dependencies.forEach(
                dependency -> {
                    ...
                    logger.info("Load dependency name[{}] version[{}] repo[{}] dependencyRepoName[{}]", dependencyKey, dependencyVersion, subPath, dependencyRepoName);
                    ...
                    try {
                        dependencyValues =
                                buildDefaultValues(
                                        ...
                                        subPath,
                                        collectorInformation);
                        collectorInformation.reportUniqKey(dependencyKey + "#" + dependencyVersion,
                                CollectorInformation.LEVEL.INFO,
                                "Load dependency name[" + dependencyKey + "] version[" + dependencyVersion + "] repo[" + dependencyRepoName + "]");
                    } catch (Exception e) {
                        ...
                        collectorInformation.reportUniqKey(dependencyRepoName + "#" + dependencyVersion,
                                CollectorInformation.LEVEL.WARNING,
                                "Load dependency name[" + dependencyKey + "] version[" + dependencyVersion + "] repo[" + dependencyRepoName + "] error:" + e.getMessage());
                        return;
                    }
                    logger.debug(
                            "Handling dependency {} - {}/{}:{}",
    ...
    public Map<String, Long> reviewValuesAsBulk(String version, List<Map<String, Object>> valuesFiles) {
        ReviewReponse result = new ReviewReponse();
        valuesFiles.forEach(
                values -> {
                    ReviewReponse singleResult = reviewMergedValues(version, values);
                    result.add(singleResult);
                });

        // Flatten all results and collect occurrences of ruleName
        return result.rulesEvaluation.stream()
     ...
     * CpmposeKey
     *
     * @param keySequence List of keys
     */
